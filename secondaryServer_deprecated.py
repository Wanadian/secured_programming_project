...
from watchDog_deprecated import communicationWithWatchDog
...
def sharedMemorySecondaryServer(sharedMemory, pathTube1, pathTube2):
    try:
        ...
        print('Taille du segment mémoire partagée en octets via second accès :',
              len(sharedMemoryC.buf[:len(sharedMemory.buf)]))
        print('Contenu du segment mémoire partagée en octets via second accès :',
              bytes(sharedMemoryC.buf[:len(sharedMemory.buf)]))
